...
class MFNET_3D(nn.Module):
    ...
    def __init__(self, num_classes, batch_size=1, pretrain=True,**kwargs):
...
if __name__ == "__main__":
    ...
    # ---------
    net = MFNET_3D(num_classes=100, batch_size=1)
    data = torch.autograd.Variable(torch.randn(1,3,16,224,224))
    data_o = torch.autograd.Variable(torch.randn(1,3,16,224,224))
    output = net(data,data_o)
    print (output.shape)
